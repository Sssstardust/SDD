---
name: sdd-generation
description: 基于 `structured-prd.json`、`需求规格.md`、`module-map.json`、`schema-context.json` 和架构约束生成 `技术方案-v{N}.md` 与最小可用 `设计包/`。当 Codex 需要从结构化需求和项目事实生成可进入 Gate 的技术设计草稿、按 `capability_tags` 联动设计包、读取 `gate-report.json` 进行定向修复，或通过 `run_pipeline.py generate-design` 自动产出设计文档时使用本 Skill。
---
...
5. 使用 [render_design_pack.py](render_design_pack.py) 生成与 `capability_tags` 对齐的最小 `设计包/` 文件集。
...
- `技术方案-v{N}.md` 必须满足当前仓库的固定章节结构，并显式引用 `设计包/`。
- `设计包/` 只生成当前 `capability_tags` 必需的文件，但每个文件都必须满足现有校验脚本的最小语义要求。
