...
exports.STANDARDS_DIR = fs.existsSync(exports.DOCS_STANDARDS_DIR) ? exports.DOCS_STANDARDS_DIR : exports.PACKAGE_STANDARDS_DIR;
exports.RULES = [
    {
        id: "layering",
...
function readRuleContent(file) {
    const rulePath = path.join(exports.STANDARDS_DIR, file);
...
function getRuleByIdOrFile(ruleIdOrFile) {
    return exports.RULES.find((rule) => rule.id === ruleIdOrFile || rule.file === ruleIdOrFile);
}
function getRule(ruleIdOrFile) {
...
function selectRules(featureType, capabilityTags, ruleFiles) {
    if (ruleFiles && ruleFiles.length > 0) {
        return exports.RULES.filter((rule) => ruleFiles.includes(rule.file) || ruleFiles.includes(rule.id));
    }
    return exports.RULES.filter((rule) => {
...
function getConstraints(ruleIds) {
    const rules = ruleIds && ruleIds.length > 0 ? exports.RULES.filter((rule) => ruleIds.includes(rule.id) || ruleIds.includes(rule.file)) : exports.RULES;
    ...
    const semantic_checks = ruleIds && ruleIds.length > 0
        ? exports.SEMANTIC_CHECKS.filter((check) => ruleIds.includes(check.id))
        : exports.SEMANTIC_CHECKS;
