...
function resolveAttachmentPath(argumentsObject: JsonRecord): string {
...
function resolveProfile(argumentsObject: JsonRecord): string | null {
  return typeof argumentsObject.profile === "string" && argumentsObject.profile.trim()
    ? argumentsObject.profile.trim()
    : null;
}

function addAttachmentContextArgs(args: string[], argumentsObject: JsonRecord): void {
  if (typeof argumentsObject.attachment_file === "string" && argumentsObject.attachment_file.trim()) {
...
function readAttachmentPayload(argumentsObject: JsonRecord): JsonRecord | null {
  return readJsonFile(resolveAttachmentPath(argumentsObject));
}

function currentAttachment(argumentsObject: JsonRecord): JsonRecord | null {
  ...
  if (profile && Array.isArray(payload.profiles)) {
    const matched = payload.profiles.find((item: any) => item && item.profile === profile);
...
function projectBucketName(argumentsObject: JsonRecord): string | null {
...
function projectArtifactsDir(argumentsObject: JsonRecord): string {
...
function resolveFeatureDirForContext(value: any, argumentsObject: JsonRecord): string {
  if (typeof value !== "string" || !value.trim()) {
    throw new Error("feature_dir is required");
  }
  ...
  const designRoots = Array.isArray(attachment?.design_roots)
    ? attachment.design_roots.filter((item: any): item is string => typeof item === "string" && item.trim().length > 0)
    : [];
...
function ensureTasksDir(argumentsObject?: JsonRecord): string {
...
function taskPaths(taskId: string, tasksDir: string = TASKS_DIR): JsonRecord {
...
function readTaskState(taskId: string, tasksDir: string = TASKS_DIR): JsonRecord | null {
  const paths = taskPaths(taskId, tasksDir);
  return readJsonFile(String(paths.task_file));
}

function startPipelineTask(pipelineArgs: string[], taskLabel?: string, argumentsObject: JsonRecord = {}): JsonRecord {
...
export function implementationSummaryFromReport(report: any): JsonRecord {
  ...
  return {
    ...
    gate5_admission_summary:
      report.gate5_admission_summary && typeof report.gate5_admission_summary === "object"
        ? report.gate5_admission_summary
        : {},
  };
}

function latestArtifactFile(fileName: string, argumentsObject: JsonRecord = {}): string | null {
  ...
  const candidates = fs
    ...
    .map((entry: fs.Dirent) => path.join(artifactsRoot, entry.name, fileName))
...
function featureArtifacts(featureDir: string, argumentsObject: JsonRecord = {}): JsonRecord {
...
export function toolDispatch(name: string, argumentsObject: JsonRecord): JsonRecord {
  ...
  if (name === "project_console_cycle") {
    ...
    return {
      ...
      strict_recommended_count:
        typeof projectConsole?.strict_recommended_count === "number" ? projectConsole.strict_recommended_count : 0,
      strict_next_step_count:
        typeof projectConsole?.strict_next_step_count === "number" ? projectConsole.strict_next_step_count : 0,
      strict_summary:
