...
def should_skip_file(filename: str, brief_text: str) -> bool:
    """如果该设计文档在当前业务描述中无实质应用，则不予物理初始化输出，防暴露模板"""
    # 过滤掉 YAML 头文件，只在实际需求描述正文中查找关键词
    body_text = brief_text
    match = re.search(r"```yaml\s*(.*?)```", brief_text, re.DOTALL | re.IGNORECASE)
    if match:
        body_text = brief_text.replace(match.group(0), "")
        
    brief_lower = body_text.lower()
    
    # 支付状态机与对账策略
    if filename in ["支付状态机.md", "对账策略.md"]:
        keywords = ["支付", "对账", "退款", "订单金额", "交易", "pay", "reconcile", "refund"]
        if not any(kw in brief_lower for kw in keywords):
            return True
            
    # 外部调用策略
    if filename in ["外部调用策略.md"]:
        keywords = ["外部调用", "三方接口", "第三方接口", "外部系统", "外联", "http client", "feign", "resttemplate"]
        if not any(kw in brief_lower for kw in keywords):
            return True
            
    # 幂等策略
    if filename in ["幂等策略.md"]:
        keywords = ["幂等", "并发", "重复提交", "分布式锁", "idempotent", "unique index"]
        if not any(kw in brief_lower for kw in keywords):
            return True
            
    # 接口契约
    if filename in ["接口契约.openapi.yaml"]:
        keywords = ["api", "接口", "url", "http", "post", "get", "swagger", "openapi"]
        if not any(kw in brief_lower for kw in keywords):
            return True
    ...
    with feature_lock(feature_dir, phase="init-design-pack"):
        for tag in tags:
            for filename in TAG_TO_FILES.get(tag, []):
                ...
                if target.exists():
                    continue
                if should_skip_file(filename, text):
                    continue
