...
def load_slice_template() -> str:
    ...
    template_path = get_active_spec_dir(root=ROOT) / "templates" / "slices" / "task-slice.template.md"
...
def ensure_design_approval(feature_dir: Path) -> int:
    """强制检查设计审批状态。"""
    command = [sys.executable, str(ROOT / "sdd_core" / "application" / "generators" / "check_approval.py"), str(feature_dir)]
    ...
    with feature_lock(feature_dir, phase="generate-task-slices"):
        ...
        for tag in tags:
            ...
            if tag == "db-change" and sql_files:
                ...
                guide_lines.append("")
                guide_lines.append("执行命令：`python sdd_core/run_pipeline.py apply-db-changes` (若项目支持)")
                guide = "\n".join(guide_lines)
