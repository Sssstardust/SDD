...
def validate_baseline_key_partition(report_dir: Path, errors: list[str]) -> None:
    script = ROOT / "sdd_core" / "application" / "generators" / "check_baseline_key_partition.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.stage in {"implementation", "all"}:
        ...
        effective_baseline_dir = get_active_baseline_dir(
            attachment_path=Path(args.attachment_file) if args.attachment_file else DEFAULT_ATTACHMENT_PATH,
            profile=args.profile,
            create=True,
            migrate_legacy=True,
        )
        validate_baseline_key_partition(effective_baseline_dir, errors)
