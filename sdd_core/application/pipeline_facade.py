...
from sdd_core.application.pipeline_execution import (
    run_captured_command,
    run_external_command as app_run_external_command,
    run_steps,
    run_steps_resilient,
)
from sdd_core.application.project_runtime import (
...
from sdd_core.application.preflight import (
    assert_feature_prerequisites,
    missing_feature_prerequisites,
)
...
from sdd_core.infrastructure.polyquery_adapter import (
    DEFAULT_CONFIG_PATH as DEFAULT_POLYQUERY_CONFIG_PATH,
)
from sdd_core.application.pipeline_result import build_result, emit_result
from sdd_core.application.pipeline_orchestration import (
...
from sdd_core.application.generators.install_sdd_runtime import (
    install_runtime as install_local_sdd_runtime,
)
from sdd_core.infrastructure.versioning import (
    detect_latest_design_path,
    get_primary_design_root,
    reports_dir_for_design,
    resolve_feature_dir,
)
...
def run_external_command(command: list[str]) -> int:
    return app_run_external_command(
        command, json_mode=JSON_MODE, traced_runner=run_traced_captured_command
    )
...
def bootstrap(feature_dir: str, force: bool = False) -> int:
    script = ROOT / "sdd_core" / "bootstrap_feature.py"
...
def generate_feature_brief(
    ...
    script = ROOT / "sdd_core" / "generate_feature_brief.py"
...
def init_design(feature_dir: str) -> int:
    script = ROOT / "sdd_core" / "init_design.py"
...
def init_design_pack(feature_brief: str) -> int:
    script = ROOT / "sdd_core" / "init_design_pack.py"
...
def init_approval(feature_dir: str) -> int:
    script = ROOT / "sdd_core" / "init_approval.py"
    return run_external_command([sys.executable, str(script), feature_dir])
...
def approve_design(
    ...
    script = ROOT / "sdd_core" / "approve_design.py"
    cmd = [
        sys.executable,
        str(script),
        feature_dir,
...
def check_approval(
    ...
    script = ROOT / "sdd_core" / "check_approval.py"
    cmd = [sys.executable, str(script), feature_dir]
    if attachment_file:
        cmd.extend(["--attachment-file", attachment_file])
    if profile:
...
def update_design_index(feature_dir: str) -> int:
    script = ROOT / "sdd_core" / "update_index_design.py"
    return run_external_command([sys.executable, str(script), feature_dir])
...
def generate_task_slices(
    ...
    script = ROOT / "sdd_core" / "generate_task_slices.py"
    cmd = [sys.executable, str(script), feature_dir]
    if attachment_file:
        cmd.extend(["--attachment-file", attachment_file])
    if profile:
...
def gate4(feature_dir: str) -> int:
    script = ROOT / "sdd_core" / "generate_test_skeleton.py"
    return run_external_command([sys.executable, str(script), feature_dir])
...
def gate5(
    ...
    script = ROOT / "sdd_core" / "gate5_admissions.py"
    cmd = [sys.executable, str(script), feature_dir]
...
def release_gate(feature_dir: str, strict: bool = False) -> int:
    script = ROOT / "sdd_core" / "release_gate.py"
    cmd = [sys.executable, str(script), feature_dir]
    if strict:
        cmd.append("--strict")
    return run_external_command(cmd)
...
def check_arch_standards_sync() -> int:
    script = ROOT / "sdd_core" / "check_arch_standards_sync.py"
...
def sync_baseline(feature_dir: str, design_version: str | None = None) -> int:
    script = ROOT / "sdd_core" / "sync_baseline.py"
    cmd = [sys.executable, str(script), feature_dir]
    if design_version:
...
def attach_project(
    ...
    script = ROOT / "sdd_core" / "attach_target_project.py"
...
def onboard_project(
    ...
    script = ROOT / "sdd_core" / "onboard_attached_project.py"
    cmd = [sys.executable, str(script)]
    if project_root:
        cmd.extend(["--project-root", project_root])
    if name:
...
def validate_reports(feature_dir: str, stage: str = "all") -> int:
    script = ROOT / "sdd_core" / "validate_reports.py"
...
def validate_all_reports(
    ...
    script = ROOT / "sdd_core" / "validate_all_reports.py"
...
def build_approval_summary(feature_dir: str) -> int:
    script = ROOT / "sdd_core" / "build_approval_summary.py"
    return run_external_command([sys.executable, str(script), feature_dir])
...
def build_flow_status(
    ...
    script = ROOT / "sdd_core" / "build_flow_status.py"
    cmd = [sys.executable, str(script), feature_dir]
    if attachment_file:
        cmd.extend(["--attachment-file", attachment_file])
    if profile:
...
def build_flow_overview(
    ...
    script = ROOT / "sdd_core" / "build_flow_overview.py"
    cmd = [sys.executable, str(script)]
    if attachment_file:
        cmd.extend(["--attachment-file", attachment_file])
    if profile:
...
def build_project_next(
    ...
    script = ROOT / "sdd_core" / "build_project_next.py"
    cmd = [sys.executable, str(script)]
    if attachment_file:
        cmd.extend(["--attachment-file", attachment_file])
    if profile:
...
def build_project_console(
    ...
    script = ROOT / "sdd_core" / "build_project_console.py"
    cmd = [sys.executable, str(script)]
    if attachment_file:
        cmd.extend(["--attachment-file", attachment_file])
    if profile:
...
def build_workspace_hygiene() -> int:
    script = ROOT / "sdd_core" / "build_workspace_hygiene.py"
    return run_external_command([sys.executable, str(script)])
...
def refresh_project_state(
    ...
    script = ROOT / "sdd_core" / "refresh_project_state.py"
    cmd = [sys.executable, str(script)]
    if feature_name:
...
def upgrade_design_tests(
    ...
    script = ROOT / "sdd_core" / "upgrade_design_tests.py"
    cmd = [sys.executable, str(script)]
    if feature_name:
        cmd.extend(["--feature", feature_name])
    if attachment_file:
...
def build_command_handlers() -> dict[str, Callable]:
    return {
        ...
        "verify": lambda args: 0,
        "init-design": lambda args: init_design(args.feature_dir),
        "generate-design": lambda args: generate_design(
        ...
        "archive-design": lambda args: archive_design(
            args.feature_name, args.intent_id, args.status, args.reason
        ),
        "sync-baseline": lambda args: sync_baseline(
            args.feature_dir, args.design_version
        ),
        "refresh-module-map": lambda args: refresh_module_map(
            args.attachment_file, args.profile
        ),
        ...
        "refresh-project-state": lambda args: refresh_project_state(
            args.feature, args.attachment_file, args.profile
        ),
        "validate-reports": lambda args: validate_reports(
            args.feature_dir,
            args.stage,
        ),
        "validate-all-reports": lambda args: validate_all_reports(
            args.stage, args.require_verify, args.attachment_file, args.profile
        ),
        ...
        "flow-status": lambda args: build_flow_status(
            args.feature_dir, args.attachment_file, args.profile
        ),
        "feature-cycle": lambda args: run_feature_cycle(args.feature_dir),
        "flow-overview": lambda args: build_flow_overview(
            args.attachment_file, args.profile
        ),
        "project-next": lambda args: build_project_next(
            args.attachment_file, args.profile
        ),
        "project-console": lambda args: build_project_console(
            args.attachment_file, args.profile
        ),
        "project-console-cycle": lambda args: run_project_console_cycle(
            args.attachment_file, args.profile
        ),
        "continue-project-flow": lambda args: run_continue_project_flow(
            args.attachment_file, args.profile
        ),
        "project-cycle": lambda args: run_project_cycle(
            args.attachment_file, args.profile
        ),
        "tooling-hygiene": lambda args: build_tooling_hygiene(),
        "workspace-hygiene": lambda args: build_workspace_hygiene(),
        "upgrade-design-tests": lambda args: upgrade_design_tests(
            args.feature, args.attachment_file, args.profile
        ),
        ...
        "design-cycle": lambda args: run_design_cycle(
            args.feature_dir, args.strict, args.attachment_file, args.profile
        ),
        "design-gates": lambda args: run_design_gates(
            args.feature_dir, args.strict, args.attachment_file, args.profile
        ),
        "implementation-gates": lambda args: run_implementation_gates(
            args.feature_dir, args.strict
        ),
        "full-flow": lambda args: run_full_flow(
            args.feature_dir, args.strict, args.attachment_file, args.profile
        ),
        "install-runtime": lambda args: install_runtime_command(
            args.target_root, args.runtime_dir, args.force
        ),
