...
def parse_nullable_value(header: str, value: str) -> bool | None:
    ...
    if not value_norm:
        return None
    if any(token in normalize_markdown_header(header) for token in ("required", "notnull")):
        if value_norm in {"yes", "true", "y", "required", "notnull"}:
            return False
        if value_norm in {"no", "false", "n", "optional"}:
            return True
    if any(token in normalize_markdown_header(header) for token in ("nullable", "null")):
        if value_norm in {"yes", "true", "y", "nullable", "allow"}:
            return True
        if value_norm in {"no", "false", "n", "notnull", "required"}:
            return False
    if any(token in value_norm for token in ["notnull", "required", "not-null"]):
        return False
    if any(token in value_norm for token in ["nullable", "optional"]):
        return True
    return None
...
def extract_table_field_specs_from_data_model(path: Path) -> dict[str, dict[str, dict[str, object]]]:
    ...
    for raw_line in text.splitlines():
        ...
        if any(cell in {"table_name", "field_name", "column", "name"} for cell in normalized_cells):
            headers = cells
            field_index = find_header_index(headers, {"table_name", "field_name", "column", "name"})
            type_index = find_header_index(headers, {"type", "datatype"})
            nullable_index = find_header_index(headers, {"nullable", "required", "notnull"})
            continue
