...
Design index lifecycle operations:
- cancel: mark the current design intent as CANCELLED
- archive: move non-ACTIVE entries into a separate archive file
"""
...
def cancel_design_intent(
    ...
    *,
    ...
    effective_baseline_dir = baseline_dir or get_active_baseline_dir(create=True, migrate_legacy=True)
    with path_lock(effective_baseline_dir, phase="design-index-cancel"):
        design_index_path, _, _ = ensure_design_index_files(effective_baseline_dir)
        index_data = read_index(design_index_path)

        target = next((item for item in index_data if item.get("intent_id") == intent_id), None)
        if target is None:
            return {"result": "error", "message": f"design intent not found: {intent_id}", "index": str(design_index_path)}

        current_status = str(target.get("status") or "")
        if current_status == "CANCELLED":
            return {"result": "noop", "message": f"design intent already CANCELLED: {intent_id}", "index": str(design_index_path)}
        if current_status != "ACTIVE":
            return {
                "result": "error",
                "message": f"only ACTIVE design intents can be cancelled, current status={current_status or 'UNKNOWN'}: {intent_id}",
...
def archive_design_entries(
    *,
    ...
    effective_baseline_dir = baseline_dir or get_active_baseline_dir(create=True, migrate_legacy=True)
    with path_lock(effective_baseline_dir, phase="design-index-archive"):
        design_index_path, _, archive_index_path = ensure_design_index_files(effective_baseline_dir)
        index_data = read_index(design_index_path)
        ...
        if not candidates:
            return {
                "result": "error",
                "message": "no matching design index entries available for archive",
                "index": str(design_index_path),
        ...
        if active_entries:
            ...
            return {
                "result": "error",
                "message": f"archiving ACTIVE design entries is forbidden: {active_intents}",
                "index": str(design_index_path),
                "archive": str(archive_index_path),
            }
...
def main() -> int:
    ...
    p_cancel.add_argument("feature_dir", help="specs/<feature> directory path")
    p_cancel.add_argument("--reason", default="", help="cancel reason")
    p_cancel.add_argument("--baseline-dir", default=None, help="baseline directory path")

    p_archive = subparsers.add_parser("archive")
    p_archive.add_argument("--feature-name", default=None, help="archive by feature name")
    p_archive.add_argument("--intent-id", default=None, help="archive by intent_id")
    p_archive.add_argument(
        "--status",
        action="append",
        choices=["ACTIVE", "SUPERSEDED", "CANCELLED", "IMPLEMENTED"],
        default=None,
        help="archive only selected statuses, repeatable",
    )
    p_archive.add_argument("--reason", default="", help="archive reason")
    p_archive.add_argument("--baseline-dir", default=None, help="baseline directory path")
    ...
    if args.action == "cancel":
        ...
        if result["result"] == "cancelled":
            print("[OK] design intent cancelled")
    ...
    if args.action == "archive":
        if not args.feature_name and not args.intent_id:
            print("[FAIL] archive requires --feature-name or --intent-id")
            return 1
        ...
        if result["result"] == "archived":
            print("[OK] design entries archived")
