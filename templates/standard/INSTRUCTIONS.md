...
Use the following semantic actions via `scripts/run_pipeline.py` or the `sdd-pipeline` MCP:

### 1. Analyze (PRD -> Structured Requirements)
Transform raw PRD into a validated `需求规格.md`.
```powershell
python scripts/run_pipeline.py analyze <source_file_path> <feature_name>
```
...
```powershell
python scripts/run_pipeline.py design <feature_name>
```
...
```powershell
python scripts/run_pipeline.py validate <feature_name>
```
...
## 📝 Troubleshooting
If the host environment blocks Node subprocesses, fall back to direct execution: `python scripts/run_pipeline.py <command>`.
